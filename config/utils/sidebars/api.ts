...
import { SectionFactory, SectionEntryProvider, SectionFileProvider, SectionDirectoryProvider, SectionMetadataProvider, SectionLinkProvider, SectionItemProvider } from "./services"
...
export function mkSubSection(sectionRoot, relativePath, collapsible = true): SidebarItemCategoryEntry {
    ...
    container.bind('IdMaker').toConstantValue((entry: string) =>
        removeLeadingSlashes(path.join(relativePath, removeNumericPrefix(removeExtension(entry))).replace(new RegExp('\\\\', 'g'), '/'))
    )
